    ...
    if (tmpText.match(determineDie)) {
      ...
      Dice.push(
        new Die(
          i,
          die_text,
          parseModifierArr(modifier),
          connectorStr,
          DieOptionsMap.get(i + '-' + die_text)
        )
      );
      die_text = undefined;
      modifier = [];
...
function buildMessage(text, dieOptions, diceOptions) {
  return new Message(
    undefined,
    text,
    buildDiceArray(text, dieOptions),
    buildDiceOptions(diceOptions),
    undefined,
    undefined,
    undefined
  );
...
function rollFuncPost(messageJSON = null) {
  ...
  if (host.includes('0.0.0.0') || host.includes('localhost') || host.includes('127.0.0.1')) {
    host = '';
  }
...
function genHTMLHistory() {
  let historyArr = getHistory();
  clearHTMLHistory();
  let historyMain = $('#historyMain');
  for (let i = historyArr.length - 1; i > -1; i--) {
    let hist = historyArr[i];
    let num = i + 1;
    let rollID = hist.rollID;
    let valText = 'Dice: ' + hist.dString + ' // ' + hist.rollText;
    ...
    // Prepend button
    let prepend = $('<div>', { class: 'input-group-prepend' }).append(
      $('<span>', { class: 'input-group-text' }).append(
        $('<button>', {
          type: 'button',
          class: 'btn btn-sm btn-outline-success',
          click: function () {
            modalHistoryRoll(rollID);
          },
        }).append(
          $(
    ...
    let input = $('<input>', {
      class: 'form-control',
      type: 'text',
      id: 'historyDiceResults-' + num,
      placeholder: valText,
      readonly: true,
    });

    // Append button
    let append = $('<div>', { class: 'input-group-append' }).append(
      $('<span>', { class: 'input-group-text' }).append(
        $('<button>', {
          type: 'button',
    ...
    let append = $('<div>', { class: 'input-group-append' }).append(
      $('<span>', { class: 'input-group-text' }).append(
        $('<button>', {
          ...
          $(
            '<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-file-minus" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4 1h8a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2zm0 1a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V3a1 1 0 0 0-1-1H4z"/><path fill-rule="evenodd" d="M5.5 8a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5z"/></svg>'
          )
        )
      )
    );
...
function genDiceDropDown() {
  ...
  $('#dieDropDownMenuText').text('Select Dice for Die Options:');
  let diceArr = getDiceArr();
  let menu = $('#dieOptionsDropDownMenu');
  for (let index = 0; index < diceArr.length; index++) {
    let diceVal = diceArr[index];
    if (!diceVal.match(determineDie)) {
      continue;
    }
    let mapVal = index + '-' + diceVal;
    ...
    DieOptionsMap.set(mapVal, DieOptionsMap.get(mapVal));
  }
}

function clearHTMLDiceDropDown() {
  $('#dieOptionsDropDownMenu').empty();
}
  ...
  if (isDiePresent(mapKey) === false) {
    ...
    for (dropDownObj of document.getElementById('dieOptionsDropDownMenu').children) {
      if (dropDownObj.firstChild.id === 'dropDown-' + mapKey) {
        ...
        document.getElementById('dieDropDownMenuText').innerText =
          'Selected Dice is: ' + dropDownObj.firstChild.innerText;
      } else {
...
function deActiveDropDowns() {
  for (let dropDownObj of document.getElementById('dieOptionsDropDownMenu').children) {
    dropDownObj.firstChild.classList.remove('active');
  }
...
function changeDie(die) {
  ...
  let searchText = '[1-9]*' + die + '$';
  let re = new RegExp(searchText);
  let tempText = document.getElementById('DiceText').value;
...
function changeField(newStr) {
  let currentText = document.getElementById('DiceText');
